...
export default function FuncionarioForm({ funcionario, onClose }) {
  ...
  const mutation = useMutation({
    mutationFn: (data) =>
      isEditing
        ? funcionariosService.atualizar(funcionario.funcionarioId, data)
        : funcionariosService.criar(data),
    onSuccess: () => {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1 col-span-2">
              ...
              <Input {...register('cargo')} placeholder="Ex: Vendedor" className="bg-zinc-800 border-zinc-700 text-white placeholder:text-zinc-500" />
              {errors.cargo && <p className="text-red-400 text-xs">{errors.cargo.message}</p>}
            </div>
          </div>
